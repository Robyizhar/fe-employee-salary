...
import { getData, saveData, detailBanner, editData } from '../../api/bannerAPI';
...
let debouncedRowBanner = debounce(detailBanner, 1000);

export const fetchData = () => {
    return  async (dispatch, getState) => {
        dispatch(startFetchingData());

        let perPage = getState().banners.perPage || '';
        let currentPage = getState().banners.currentPage || 1;
        let keyword = getState().banners.keyword || '';
...
export const storeData = () => {
    return async (dispatch, getState) => {
        try {
            let token = getState().auth.token || '';
            let data = getState().banners.input || [];
...
export const getBannerRow = (id) => {
    return async (dispatch, getState) => {
        try {
            let token = getState().auth.token || '';
            return await debouncedRowBanner(id, token);
        } catch (err) {
