        ...
        <Fragment>
            <div className='row'>
                ...
                <div className='col-6 col-sm-6 col-md-6 my-3'>
                    <Link to="/departement/form" className="btn btn-primary float-right" style={{float: 'right'}}>Add</Link>
                </div>
                                ...
                                {<tbody>
                                    { response.status === 'success' && response.data.data && response.data.data.map((departement, index) => 
                                        <tr key={departement.id}>
                                            ...
                                            <td>
                                                <Button onAction={() => editHandler(departement.id)} text={`Edit`} icon={`bi bi-pencil-square`} property={`btn btn-sm btn-light`}></Button>
                                                <Button onAction={() => detailHandler(departement.id)} text={`Detail`} icon={`bi bi-info-circle`} property={`btn btn-sm btn-light mx-2`}></Button>
                                                <Button onAction={() => deleteHandler(departement.id)} text={`Delete`} icon={`bi bi-trash-fill`} property={`btn btn-sm btn-light`}></Button>
                                            </td>
                                        </tr>
                                    )}
